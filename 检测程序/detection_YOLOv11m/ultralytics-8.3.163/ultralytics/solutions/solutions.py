...
class BaseSolution:
    ...
    def __init__(self, is_cli: bool = False, **kwargs: Any) -> None:
        ...
        self.LOGGER = LOGGER  # Store logger object to be used in multiple solution classes.txt
        ...
        self.region = self.CFG["region"]  # Store region data for other classes.txt usage
        self.line_width = self.CFG["line_width"]

        # Load Model and store additional information (classes.txt, show_conf, show_label)
        if self.CFG["model"] is None:
            self.CFG["model"] = "yolo11n.pt"
        self.model = YOLO(self.CFG["model"])
        self.names = self.model.names
        self.classes = self.CFG["classes.txt"]
