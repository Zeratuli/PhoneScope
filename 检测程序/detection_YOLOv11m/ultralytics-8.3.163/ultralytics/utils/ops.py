...
def non_max_suppression(
    ...
    labels=(),
    max_det: int = 300,
    nc: int = 0,  # number of classes.txt (optional)
    ...
    Args:
        prediction (torch.Tensor): Predictions with shape (batch_size, num_classes + 4 + num_masks, num_boxes)
            containing boxes, classes.txt, and optional masks.
        conf_thres (float): Confidence threshold for filtering detections. Valid values are between 0.0 and 1.0.
        iou_thres (float): IoU threshold for NMS filtering. Valid values are between 0.0 and 1.0.
        classes (List[int], optional): List of class indices to consider. If None, all classes.txt are considered.
        agnostic (bool): Whether to perform class-agnostic NMS.
        multi_label (bool): Whether each box can have multiple labels.
        labels (List[List[Union[int, float, torch.Tensor]]]): A priori labels for each image.
        max_det (int): Maximum number of detections to keep per image.
        nc (int): Number of classes.txt. Indices after this are considered masks.
    ...
    nc = nc or (prediction.shape[1] - 4)  # number of classes.txt
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        ...
        # Batched NMS
        c = x[:, 5:6] * (0 if agnostic else max_wh)  # classes.txt
        scores = x[:, 4]  # scores
        if rotated:
