...
class ConfusionMatrix(DataExportMixin):
    ...
    Attributes:
        ...
        names (List[str]): The names of the classes.txt, used as labels on the plot.
    """
    ...
    def __init__(self, names: List[str] = [], task: str = "detect"):
        ...
        Args:
            names (List[str], optional): Names of classes.txt, used as labels on the plot.
            task (str, optional): Type of task, either 'detect' or 'classify'.
        """
        self.task = task
        self.nc = len(names)  # number of classes.txt
        self.matrix = np.zeros((self.nc + 1, self.nc + 1)) if self.task == "detect" else np.zeros((self.nc, self.nc))
        self.names = names  # name of classes.txt
    ...
    def plot(self, normalize: bool = True, save_dir: str = "", on_plot=None):
        ...
        if self.nc >= 100:  # downsample for large class count
            ...
            n = (self.nc + k - 1) // k  # number of retained classes.txt
    ...
    def summary(self, normalize: bool = False, decimals: int = 5) -> List[Dict[str, float]]:
        ...
        Returns:
            (List[Dict[str, float]]): A list of dictionaries, each representing one predicted class with corresponding values for all actual classes.txt.
...
def plot_pr_curve(
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes.txt
        for i, y in enumerate(py.T):
            ax.plot(px, y, linewidth=1, label=f"{names[i]} {ap[i, 0]:.3f}")  # plot(recall, precision)
    else:
        ax.plot(px, py, linewidth=1, color="grey")  # plot(recall, precision)

    ax.plot(px, py.mean(1), linewidth=3, color="blue", label=f"all classes.txt {ap[:, 0].mean():.3f} mAP@0.5")
...
def plot_mc_curve(
    ...
    ax.plot(px, y, linewidth=3, color="blue", label=f"all classes.txt {y.max():.2f} at {px[y.argmax()]:.3f}")
...
def ap_per_class(
    ...
    Args:
        ...
        pred_cls (np.ndarray): Array of predicted classes.txt of the detections.
        target_cls (np.ndarray): Array of true classes.txt of the detections.
    ...
    Returns:
        ...
        unique_classes (np.ndarray): An array of unique classes.txt that have data.
    ...
    # Find unique classes.txt
    unique_classes, nt = np.unique(target_cls, return_counts=True)
    nc = unique_classes.shape[0]  # number of classes.txt, number of detections
    ...
    names = {i: names[k] for i, k in enumerate(unique_classes) if k in names}  # dict: only classes.txt that have data
    if plot:
...
class Metric(SimpleClass):
    ...
    Attributes:
        ...
        all_ap (list): AP scores for all classes.txt and all IoU thresholds. Shape: (nc, 10).
        ap_class_index (list): Index of class for each AP score. Shape: (nc,).
        nc (int): Number of classes.txt.

    Methods:
        ap50(): AP at IoU threshold of 0.5 for all classes.txt. Returns: List of AP scores. Shape: (nc,) or [].
        ap(): AP at IoU thresholds from 0.5 to 0.95 for all classes.txt. Returns: List of AP scores. Shape: (nc,) or [].
        mp(): Mean precision of all classes.txt. Returns: Float.
        mr(): Mean recall of all classes.txt. Returns: Float.
        map50(): Mean AP at IoU threshold of 0.5 for all classes.txt. Returns: Float.
        map75(): Mean AP at IoU threshold of 0.75 for all classes.txt. Returns: Float.
        map(): Mean AP at IoU thresholds from 0.5 to 0.95 for all classes.txt. Returns: Float.
    ...
    def ap50(self) -> Union[np.ndarray, List]:
        """
        Return the Average Precision (AP) at an IoU threshold of 0.5 for all classes.txt.
    ...
    def ap(self) -> Union[np.ndarray, List]:
        """
        Return the Average Precision (AP) at an IoU threshold of 0.5-0.95 for all classes.txt.
    ...
    def mp(self) -> float:
        """
        Return the Mean Precision of all classes.txt.

        Returns:
            (float): The mean precision of all classes.txt.
        """
    ...
    def mr(self) -> float:
        """
        Return the Mean Recall of all classes.txt.

        Returns:
            (float): The mean recall of all classes.txt.
        """
    ...
    def update(self, results: tuple):
        ...
        Args:
            results (tuple): A tuple containing evaluation metrics:
                ...
                - all_ap (list): AP scores for all classes.txt and all IoU thresholds.
...
class DetMetrics(SimpleClass, DataExportMixin):
    ...
    Attributes:
        ...
        stats (Dict[str, List]): A dictionary containing lists for true positives, confidence scores, predicted classes.txt, target classes.txt, and target images.
...
class SegmentMetrics(DetMetrics):
    """
    Calculate and aggregate detection and segmentation metrics over a given set of classes.txt.
...
class PoseMetrics(DetMetrics):
    """
    Calculate and aggregate detection and pose metrics over a given set of classes.txt.
    ...
    Methods:
        ...
        mean_results(): Return the mean of the detection and segmentation metrics over all the classes.txt.
        ...
        ap_class_index: Return the list of indices of classes.txt used to compute Average Precision (AP).
...
class ClassifyMetrics(SimpleClass, DataExportMixin):
    ...
    def process(self, targets: torch.Tensor, pred: torch.Tensor):
        """
        Process target classes.txt and predicted classes.txt to compute metrics.

        Args:
            targets (torch.Tensor): Target classes.txt.
            pred (torch.Tensor): Predicted classes.txt.
        """
