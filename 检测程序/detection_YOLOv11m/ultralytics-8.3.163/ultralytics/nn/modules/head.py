...
class Detect(nn.Module):
    ...
    Examples:
        Create a detection head for 80 classes.txt
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        """
        Initialize the YOLO detection layer with specified number of classes.txt and channels.
        ...
        """
        super().__init__()
        self.nc = nc  # number of classes.txt
    ...
    def bias_init(self):
        ...
        for a, b, s in zip(m.cv2, m.cv3, m.stride):  # from
            ...
            b[-1].bias.data[: m.nc] = math.log(5 / m.nc / (640 / s) ** 2)  # cls (.01 objects, 80 classes.txt, 640 img)
        if self.end2end:
...
class OBB(Detect):
    ...
    def __init__(self, nc: int = 80, ne: int = 1, ch: Tuple = ()):
        """
        Initialize OBB with number of classes.txt `nc` and layer channels `ch`.
...
class Classify(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, k: int = 1, s: int = 1, p: Optional[int] = None, g: int = 1):
        ...
        Args:
            ...
            c2 (int): Number of output classes.txt.
...
class WorldDetect(Detect):
    ...
    def __init__(self, nc: int = 80, embed: int = 512, with_bn: bool = False, ch: Tuple = ()):
        """
        Initialize YOLO detection layer with nc classes.txt and layer channels ch.
    ...
    def bias_init(self):
        ...
        for a, b, s in zip(m.cv2, m.cv3, m.stride):  # from
            a[-1].bias.data[:] = 1.0  # box
            # b[-1].bias.data[:] = math.log(5 / m.nc / (640 / s) ** 2)  # cls (.01 objects, 80 classes.txt, 640 img)
...
class v10Detect(Detect):
    ...
    Methods:
        __init__: Initialize the v10Detect object with specified number of classes.txt and input channels.
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        """
        Initialize the v10Detect object with the specified number of classes.txt and input channels.
