...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        Args:
            ...
            nc (int, optional): Number of classes.txt.
...
class ClassificationModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n-cls.yaml", ch=3, nc=None, verbose=True):
        """
        Initialize ClassificationModel with YAML, channels, number of classes.txt, verbose flag.
    ...
    def reshape_outputs(model, nc):
        ...
        Args:
            ...
            nc (int): New number of classes.txt.
        """
...
class RTDETRDetectionModel(DetectionModel):
    ...
    Attributes:
        nc (int): Number of classes.txt for detection.
...
class WorldModel(DetectionModel):
    ...
    Attributes:
        txt_feats (torch.Tensor): Text feature embeddings for classes.txt.
        clip_model (torch.nn.Module): CLIP model for text encoding.

    Methods:
        __init__: Initialize YOLOv8 world model.
        set_classes: Set classes.txt for offline inference.
    ...
    def set_classes(self, text, batch=80, cache_clip_model=True):
        """
        Set classes.txt in advance so that model could do offline-inference without clip model.
...
class YOLOEModel(DetectionModel):
    ...
    Attributes:
        pe (torch.Tensor): Prompt embeddings for classes.txt.
        clip_model (torch.nn.Module): CLIP model for text encoding.
    ...
    def set_classes(self, names, embeddings):
        ...
        assert not hasattr(self.model[-1], "lrpc"), (
            "Prompt-free model does not support setting classes.txt. Please try with Text/Visual prompt models."
        )
...
class SafeClass:
    """A placeholder class to replace unknown classes.txt during unpickling."""
...
class SafeUnpickler(pickle.Unpickler):
    """Custom Unpickler that replaces unknown classes.txt with SafeClass."""
...
def torch_safe_load(weight, safe_only=False):
    ...
    Args:
        ...
        safe_only (bool): If True, replace unknown classes.txt with SafeClass during loading.
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes.txt (i.e. for Classify() output)
