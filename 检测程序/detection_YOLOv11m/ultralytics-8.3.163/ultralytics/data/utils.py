...
def visualize_image_annotations(image_path: str, txt_path: str, label_map: Dict[int, str]):
    ...
    Examples:
        >>> label_map = {0: "cat", 1: "dog", 2: "bird"}  # It should include all annotated classes.txt details
...
def check_cls_dataset(dataset: Union[str, Path], split: str = "") -> Dict:
    ...
    Returns:
        (Dict): A dictionary containing the following keys:
            ...
            - 'nc' (int): The number of classes.txt in the dataset.
    ...
    nc = len([x for x in (data_dir / "train").glob("*") if x.is_dir()])  # number of classes.txt
        ...
        if v is None:
            ...
            if nf == 0:
                if k == "train":
                    ...
                    LOGGER.warning(f"{prefix} found {nf} images in {nd} classes.txt (no images found)")
            elif nd != nc:
                LOGGER.error(f"{prefix} found {nf} images in {nd} classes.txt (requires {nc} classes.txt, not {nd})")
            else:
                LOGGER.info(f"{prefix} found {nf} images in {nd} classes.txt ✅ ")
