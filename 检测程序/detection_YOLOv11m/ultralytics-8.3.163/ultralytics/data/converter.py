...
def convert_segment_masks_to_yolo_seg(masks_dir: str, output_dir: str, classes: int):
    ...
    Args:
        ...
        classes (int): Total classes.txt in the dataset i.e. for COCO classes.txt=80

    Examples:
        >>> from ultralytics.data.converter import convert_segment_masks_to_yolo_seg

        The classes.txt here is the total classes.txt in the dataset, for COCO dataset we have 80 classes.txt
        >>> convert_segment_masks_to_yolo_seg("path/to/masks_directory", "path/to/output/directory", classes.txt=80)
    ...
    for mask_path in Path(masks_dir).iterdir():
        if mask_path.suffix in {".png", ".jpg"}:
            ...
            unique_values = np.unique(mask)  # Get unique pixel values representing different classes.txt
            yolo_format_data = []
